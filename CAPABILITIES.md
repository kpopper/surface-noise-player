...
- If neither a file nor embedded artwork is available, and the release has both album artist and album title metadata, cover art is fetched automatically from MusicBrainz Cover Art Archive; the earliest release date is preferred; the image is saved as `cover.jpg` in the release folder
- If album artist or album title metadata is absent, the MusicBrainz lookup is skipped
