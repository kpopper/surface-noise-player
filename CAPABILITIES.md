...
- A release already known to the database and still present on disk is left untouched by a sync — its tracks, metadata, and artwork are not re-scanned, unless the user explicitly rescans it (see Release screen)
...
- If the first track was already locally available before a scan/rescan needed to read its tags, it is left alone afterwards instead of being evicted — a scan never evicts a track it didn't itself download
...
- Album artwork for a newly discovered release is resolved from a folder image file first (no download needed), then from the first track's embedded artwork once it has downloaded, then a MusicBrainz lookup, then an iTunes Search API lookup (see Album art)
...
- Among same-titled search results, a release-group typed as an Album is preferred over a single or EP sharing the exact same title (e.g. a promotional single released ahead of an album of the same name) before applying the earliest-date tiebreak — falling back to every candidate when none is typed as an Album
- If MusicBrainz has no match (or no cover art for its match), an iTunes Search API lookup is attempted as a further fallback, using the same album artist/title (and the same track-artist-tag fallback when album artist is absent), saving the result as `cover.jpg` in the release folder — preferring a result whose album name and artist both match what was searched for, falling back to the top result otherwise
- If a release still has no artwork, opening its release screen makes a single further attempt (a folder-image recheck, then the first track's embedded artwork, then MusicBrainz, then iTunes — falling back to a fresh read of the first track's own artist tag first if the release has no stored album artist to search with) — not retried again while the screen stays open, and not attempted automatically in the background otherwise
- An artwork retry that needs to check the first track's embedded artwork downloads it only if it isn't already locally available, and only evicts it afterward if it downloaded it itself (see Library scanning)
- Tapping the refresh button also retries artwork resolution for every currently-known, fully-scanned release that still has none, in addition to the regular directory sync (see Library screen) — this only happens on an explicit refresh, never automatically on launch; each release goes through the same folder-image, then embedded-artwork, then MusicBrainz, then iTunes order as the single on-demand retry
- MusicBrainz lookups are serialized to at most one request per second (matching its published rate limit), and iTunes lookups to at most one every three seconds (matching its ~20/minute published limit), regardless of how many releases are being resolved at once — so a refresh retrying artwork for many releases may take a while to work through all of them
...
- The app bar has a manual rescan button; tapping it treats the file tags as the source of truth — it re-reads the first track's album artist/album title tags and re-resolves artwork through the same folder-image, then embedded-artwork, then MusicBrainz, then iTunes order used for a new release, overwriting the release's stored name, album title, album artist, and art path with whatever those tags now say, even if something different was already stored
- While a manual rescan is in progress, the rescan button is replaced by a spinner
- A manual rescan does not touch individual track titles/artists — those still only update the next time each track is played (see Audio metadata)
