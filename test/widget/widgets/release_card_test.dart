...
Release makeRelease(
  ...
  return Release(
      folderPath: '/music/album', name: name, tracks: tracks, tags: tags);
}

void main() {
  testWidgets('displays the release name', (tester) async {
    await tester.pumpWidget(wrap(
        ReleaseCard(release: makeRelease(name: 'Dark Side'), onTap: () {})));
    expect(find.text('Dark Side'), findsOneWidget);
  });

  testWidgets('shows track count when no tags', (tester) async {
    await tester.pumpWidget(
        wrap(ReleaseCard(release: makeRelease(trackCount: 9), onTap: () {})));
  ...
  testWidgets('does not show track count when tags are present',
      (tester) async {
  ...
  testWidgets('calls onTap when tapped', (tester) async {
    bool tapped = false;
    await tester.pumpWidget(
        wrap(ReleaseCard(release: makeRelease(), onTap: () => tapped = true)));
  ...
  testWidgets('shows a trailing chevron icon', (tester) async {
    await tester
        .pumpWidget(wrap(ReleaseCard(release: makeRelease(), onTap: () {})));
